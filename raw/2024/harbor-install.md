...
> 使用 Docker Compose 部署 Harbor 私有镜像仓库，包含自签名 SSL 证书生成、YAML 配置、Nginx 反向代理等完整流程。
...
尚未安装 Docker？参见：[Docker 安装与配置指南](/2022/docker-guide/)

Docker Compose 安装与使用参见：[Docker Compose 安装配置](/2022/docker-compose-guide/)
...
<tbody>
  <tr>
    ...
    <td>
      Harbor 访问域名或 IP（本文用 <code code="harbor.local">
        harbor.local
      </code>
  ...
  <tr>
    <td>
      <code code="https.certificate">
        https.certificate
      </code>
    </td>
    
    <td>
      CRT 证书路径（建议 <code code="/opt/harbor/crt/harbor.crt">
        /opt/harbor/crt/harbor.crt
      </code>
  ...
  <tr>
    <td>
      <code code="https.private_key">
        https.private_key
      </code>
    </td>
    
    <td>
      KEY 私钥路径（建议 <code code="/opt/harbor/crt/harbor.key">
        /opt/harbor/crt/harbor.key
      </code>
  ...
  <tr>
    <td>
      <code code="harbor_admin_password">
        harbor_admin_password
      </code>
    </td>
    
    <td>
      admin 初始密码（默认 <code code="Harbor12345">
        Harbor12345
      </code>
...
<alert title="务必修改默认密码 Harbor12345" type="warning">
...
<alert title="推荐在第一次 install.sh 之前就调整好的配置" type="info">
...
<alert title="external_url 详解" type="question">

external_url 是 Harbor 配置文件中的参数，用于定义外部客户端访问该服务的基准 URL。其技术逻辑如下：

优先级：启用该参数后，配置中的 hostname 将失效。<br />


令牌生成：Harbor 的认证服务（Token Service）会根据此参数生成 OAuth 令牌挑战（Challenge）中的 realm 地址。<br />
...
<alert title="如果您无法拉取" type="warning">

如果您的客户端无法拉取或上传，可以参考这篇文章配置：

<link-card description="在 Linux 和 Windows 客户端上配置 Docker 信任私有镜像仓库的自签名 SSL 证书，解决拉取推送时的证书验证失败问题" icon="https://picsum.photos/100/100" link="/2025/docker-private-cert" title="Docker 配置私服自签名证书信任">
...
<alert title="关于双 TLS 架构" type="question">
...
不知道为什么，这个在新的docker版本无法使用，必须https
...
<alert title="HTTP 模式的真实风险" type="warning">
...
<tbody>
  <tr>
    <td>
      <code code="docker login">
        docker login
      </code>
      
       报 <code code="x509: certificate signed by unknown authority">
    ...
    <td>
      ...
      ：登录 <code code="harbor.local">
        harbor.local
      </code>
      
       就放 <code code="/etc/docker/certs.d/harbor.local/ca.crt">
  ...
  <tr>
    <td>
      <code code="docker push">
        docker push
      </code>
      
       报 <code code="x509: cannot validate certificate for 192.168.x.x because it doesn't contain any IP SANs">
    ...
    <td>
      在 <code code="harbor_cert.cnf">
        harbor_cert.cnf
      </code>
      
       的 <code code="[alt_names]">
        [alt_names]
      </code>
      
       段加 <code code="IP.1 = 192.168.x.x">
        IP.1 = 192.168.x.x
      </code>
  ...
  <tr>
    <td>
      <code code="docker push">
       ...
       大镜像报 <code code="413 Request Entity Too Large">
        413 Request Entity Too Large
      </code>
    </td>
    
    <td>
      Nginx <code code="client_max_body_size">
    ...
    <td>
      确认 Nginx 配置里有 <code code="client_max_body_size 0;">
  ...
  <tr>
    <td>
      登录报 <code code="unauthorized: authentication required">
    ...
    <td>
      <code code="external_url">
       ...
       配置错，Harbor 返回的 token realm 对不上实际访问地址
    </td>
    
    <td>
      核对 <code code="external_url">
        external_url
      </code>
      
       协议 / 域名 / 端口和浏览器地址栏一致，改完跑 <code code="./prepare">
        ./prepare
      </code>
    </td>
  </tr>
  ...
  <tr>
    ...
    <td>
      Nginx upstream 端口不对 / 没关 <code code="proxy_ssl_verify">
        proxy_ssl_verify
      </code>
    </td>
    
    <td>
      确认 <code code="harbor.yml">
        harbor.yml
      </code>
      
       的 <code code="https.port">
        https.port
      </code>
      
       和 Nginx <code code="upstream">
        upstream
      </code>
      
       端口一致；加 <code code="proxy_ssl_verify off;">
        proxy_ssl_verify off;
      </code>
    </td>
  </tr>
  ...
  <tr>
    <td>
      <code code="docker compose ps">
        docker compose ps
      </code>
      
       某容器反复 <code code="Restarting">
        Restarting
      </code>
    </td>
    
    <td>
      端口冲突 / <code code="harbor.yml">
        harbor.yml
      </code>
    ...
    <td>
      <code code="docker compose logs <服务名>">
  ...
  <tr>
    <td>
      hosts 配好了但 <code code="ping harbor.local">
        ping harbor.local
      </code>
    ...
    <td>
      写错了行 / hosts 文件有 BOM / Windows 权限不够
    </td>
    
    <td>
      <code code="cat /etc/hosts">
        cat /etc/hosts
      </code>
  ...
  <tr>
    ...
    <td>
      确认 <code code="proxy_buffering off; proxy_request_buffering off;">
        proxy_buffering off; proxy_request_buffering off;
      </code>
      
      ；调大 <code code="proxy_send_timeout">
        proxy_send_timeout
      </code>
      
       / <code code="proxy_read_timeout">
        proxy_read_timeout
      </code>
    </td>
  </tr>
...
<alert title="升级失败的回滚思路" type="question">
